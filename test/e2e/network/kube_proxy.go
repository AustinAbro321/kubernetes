...
var _ = common.SIGDescribe("KubeProxy", func() {
	...
	framework.It("should proxy localhost NodePort traffic to backends", framework.WithFeatureGate(features.LocalhostNodePortUserspaceProxy), func(ctx context.Context) {
		cs := fr.ClientSet
		ns := fr.Namespace.Name
